...
export function MuscleCar({ color, finish, shader, shaderParams }) {
  const { scene } = useGLTF('/models/conceptcar033.glb')
  ...
  useEffect(() => {
    scene.traverse((child) => {
      if (child.isMesh && (child.name === 'Object_11' || child.material?.name === 'body_color_supra.001')) {
  ...
  return (
    <group dispose={null} position={[0, 1.109, 0]}>
      <primitive object={scene} />
    </group>
  )
}

useGLTF.preload('/models/conceptcar033.glb')
